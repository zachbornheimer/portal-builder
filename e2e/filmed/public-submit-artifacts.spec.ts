...
const DEFINITION_ROUTE = (id: string) => `/wp-json/dragongate/v1/portals/${id}/definition`
  ...
  try {
    ...
    try {
      ...
      await test.step('submit and wait for success', async () => {
        ...
        await expect(publicPage.locator('[data-dg-submit-status="success"]')).toBeVisible()
      })
    } finally {
    ...
    await test.step('assert sheet row and drive files for this portal', async () => {
      ...
      expect(fs.existsSync(sheet), `missing sheet ${sheet}\n${TEST_MODE_HELP}`).toBeTruthy()
      expect(
      ...
      expect(
        driveFiles.some((name) => /recording/i.test(name) && name.endsWith('.mp3')),
      ).toBeTruthy()
