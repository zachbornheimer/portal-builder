...
if ( ! class_exists( 'Data_Table' ) ) {
	...
	class Data_Table {
		...
		$row_html = '<div id="' . esc_attr( $this->table_id ) . '-row-' . esc_attr( $row_index ) . '" class="grow p-6 lg:rounded-lg lg:bg-white lg:p-10 lg:shadow-xs lg:ring-1 lg:ring-gray-200 dark:lg:bg-gray-900 dark:lg:ring-gray-700 flex items-start justify-between mb-4">';
			...
			if ( $block ) {
				// Block mode: content on the left, delete button on the right
				...
				foreach ( $this->columns as $index => $column ) {
					...
					$block_heading = '<label class="block text-sm font-medium text-gray-700 mb-2">' . $column . '</label>';
					// Add copy button next to the "Columns" label when in block mode
					if ( $index === 2 && $column === 'Columns' ) {
						$block_heading = '<label class="block text-sm font-medium text-gray-700 mb-0 inline">' . $column . '</label> <button type="button" class="button button-secondary copy-columns ml-2 inline">Copy</button>';
					}
					// Add Google Sheet link next to the "Google Sheet ID" label when in block mode
					if ( $index === 1 && $column === 'Google Sheet ID' && !empty($value) ) {
						$sheet_url = 'https://docs.google.com/spreadsheets/d/' . esc_attr($value);
						$block_heading = '<label class="block text-sm font-medium text-gray-700 mb-2 inline">' . $column . '</label> <a href="' . $sheet_url . '" target="_blank" rel="noopener noreferrer" class="mb-2 text-blue-600 hover:text-blue-800 inline-flex items-center gap-1 ml-2">View Sheet <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg></a>';
					}
					
					$row_html .= '<div class="mb-4"><div class="mb-0 flex align-center items-center flex-row gap-2">' . $block_heading . '</div>';
					...
					if ( isset( $this->column_options[ $index ]['tags'] ) && $this->column_options[ $index ]['tags'] === true ) {
						...
						$row_html .= '<div class="text-xs text-gray-500 mt-2">';
						$row_html .= '<code class="bg-gray-200 px-1 rounded">Enter</code> to add your tag. Use commas to add multiple tags at once. <code class="bg-gray-200 px-1 rounded">Alt Shift A</code> adds all names from the content. Tag options: <code class="bg-gray-200 px-1 rounded">receipt_link</code>, <code class="bg-gray-200 px-1 rounded">drive_link</code>, <code class="bg-gray-200 px-1 rounded">app_id</code>. {{ }} wrappers are automatically added/removed.';
						$row_html .= '</div>';
						...
						if ( $index === 1 && $column === 'Google Sheet ID' && $this->disclosure ) {
						...
						if ( $index === 1 && $column === 'Google Drive Folder ID' && $this->disclosure ) {
							$row_html .= '<div class="text-xs text-gray-500 mt-1">';
							$row_html .= $this->disclosure;
							$row_html .= '</div>';
						}
